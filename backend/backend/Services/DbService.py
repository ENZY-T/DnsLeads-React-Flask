...
def DbGetMany(tableStr, columnStr=None, searchKey=None):
  ...
  result = []
  resultProxy = db.session.execute(f"SELECT * FROM {tableStr} { f'WHERE {columnStr}={searchKey}' if (columnStr and searchKey) else ''}")
...
def DbDelMany(tableStr, columnStr, searchKey):
  ...
  resultProxy = db.session.execute(f'DELETE FROM {tableStr} WHERE {columnStr}={searchKey}')
  print(resultProxy)